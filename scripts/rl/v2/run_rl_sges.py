...
import numpy as np
from strategies_v2 import SGES
...
class RLController(object):
    def __init__(self):
        ...
        # Initialize solver
        self.solver = SGES(self.sigma, self.min_alpha, self.max_alpha, self.alpha_step, 
                        self.learning_rate, self.pop_size, self.elite_size, self.warm_up, 
                        self.seed, adapt_alpha=True)
    ...
    def train(self):
        losses = []
        self.solver.initialize(np.zeros_like(self.params), self.k)
        for iter in range(self.max_iters):
            ...
            if (iter + 1) % 10 == 0:
                print("Iters: %d, Average Loss: %f, Alpha: %f" % (iter + 1, losses[-1], self.solver.alpha))
        ...
        # Save to .npy
        file_name = "SGES_%s_%s.npy" % (str(self.env_name), str(self.seed))
        try:
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iters', type=int, default=int(6e2))
    ...
    parser.add_argument('--max_alpha', type=float, default=0.8)
    parser.add_argument('--alpha_step', type=float, default=1.05)
    parser.add_argument('--learning_rate', type=float, default=0.003)
    parser.add_argument('--pop_size', type=int, default=32)
    parser.add_argument('--elite_size', type=int, default=24)
    parser.add_argument('--warm_up', type=int, default=48)
