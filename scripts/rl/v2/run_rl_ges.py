...
class RLController(object):
    def __init__(self):
        ...
        # Initialize solver
        self.solver = GES(self.sigma, self.alpha, self.learning_rate, self.pop_size, self.elite_size, 
                        self.warm_up, self.seed)

        print("Initialization of GES complete.")
    ...
    def train(self):
        ...
        # Save to .npy
        file_name = "GES_%s_%s.npy" % (str(self.env_name), str(self.seed))
        try:
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--warm_up', type=int, default=24)
